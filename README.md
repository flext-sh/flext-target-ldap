...
FLEXT Target for LDAP directory loading
...
2. Confirm parent MRO chain via `pyproject.toml` `dependencies` filtered by `flext-*` (excluding `flext-target-ldap` self).
...
- Public extensions exposed by this project: `FlextTargetLdapClient`, `FlextTargetLdapTypeConverter`, `FlextTargetLdapApiService`, `FlextTargetLdapError`, `FlextTargetLdapOrchestrator`, `FlextTargetLdapUtilities` (+31 more).
